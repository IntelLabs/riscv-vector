...
//  val replayMetaValid = RegEnable(metaCounter < totalCounter, underReplay)
  ...
  state := MuxLookup(state, state)(
    Seq(
      ...
      mode_replay       -> Mux(metaCounter >= totalCounter, mode_wait_replace, state),
      mode_send_replace -> mode_wait_replace,
  ...
  // replay output
  io.toPipe.bits.nextCycleWb := underReplay && (metaCounter < totalCounter) && !io.innerIO.bits.meta.rwType
...
class MSHRFile extends Module() {
  ...
  val allocReq  = reqArb.io.in(2).ready
  ...
  reqArb.io.out.ready := io.pipelineReq.valid || 
                          (io.fromRefill.valid && replayReg.io.innerIO.ready) ||
                          io.probeCheck.valid

  io.fromRefill.ready        := reqArb.io.in(1).ready
  replayReg.io.innerIO.valid := reqArb.io.in(1).ready
  ...
  val reqLineAddr = Mux(probeReq, io.probeCheck.lineAddr, io.pipelineReq.bits.lineAddr)
  ...
  val mshrs = (0 until nMSHRs) map {
    i =>
      ...
      mshr.io.req         := Mux((allocReq && !allocateArb.io.in(i).ready) || (replayReq && io.fromRefill.bits.entryId =/= i.asUInt), MSHRReqType.invalid, reqArb.io.out.bits)
...
//      mshr.io.maskConflict := maskConflict
  ...
  io.pipelineReq.ready     := allocReq && (!lineAddrMatch || (lineAddrMatch && !stallReq))
