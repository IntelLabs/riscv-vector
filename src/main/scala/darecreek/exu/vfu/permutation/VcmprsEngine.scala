...
class VcmprsEngine(implicit p: Parameters) extends VFuModule {
  ...
  val vd_idx_reg = RegNext(io.vd_idx)
  val vs2 = io.vs2
  ...
  val update_vs_idx_reg = RegNext(update_vs_idx)
  val cmprs_rd_old_vd_reg = RegNext(cmprs_rd_old_vd)
  val old_vd_reg = RegNext(old_vd)
  val vs2_reg = RegNext(vs2)
  ...
  val vmask_16b_reg = RegEnable(vmask_16b, 0.U, update_vs_idx)
