...
class VFPUWrapper(implicit p: Parameters) extends VFuModule {
  ...
  io.out.bits.fflags := Mux(vstart_gte_vl, 0.U, Mux(output_en, red_fflag, Mux(io.out.bits.uop.ctrl.narrow_to_1, cmp_fflag, fpu(0).io.out.bits.fflags | fpu(1).io.out.bits.fflags)))
