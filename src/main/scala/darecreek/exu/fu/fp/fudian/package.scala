...
package object fudian {
  ...
  def ROD: UInt = 5.U(3.W) // vector extension specific
  ...
  class FloatPoint(val expWidth: Int, val precision: Int) extends Bundle {
    ...
    def negate: FloatPoint = {
      val fp = Wire(new FloatPoint(this.expWidth, this.precision))
      fp.sign := !this.sign
      fp.exp := this.exp
      fp.sig := this.sig
      fp
  ...
  // without the numbers before the dot.
  object FloatPoint {
