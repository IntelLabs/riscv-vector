...
class VStdFreeList(size: Int) extends VBaseFreeList(size) {

  val freeList = RegInit(VecInit(Seq.tabulate(size)( i => (i + 32).U(VPRegIdxWidth.W) )))
  ...
  io.canAllocate := RegNext(freeRegCnt >= VRenameWidth.U) // use RegNext for better timing
  // XSDebug(p"freeRegCnt: $freeRegCnt\n")

  val allocatePtr = (0 until VRenameWidth).map(i => headPtr + i.U)
  ...
  headPtr := Mux(io.flush, FreeListPtr(!tailPtrNext.flag, tailPtrNext.value),
                 headPtrNext)
  ...
  // Debug
  val almostFull = freeRegCnt < 4.U
  if (debug) {
    dontTouch(freeRegCnt)
    dontTouch(almostFull)
  }
}
