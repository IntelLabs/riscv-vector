...
class SRAMBundleAW[T <: Data](private val gen: T, set: Int, val way: Int = 1, val useBitmask: Boolean = false)
  ...
  def apply(data: Vec[T], setIdx: UInt, waymask: UInt, bitmask: UInt): SRAMBundleAW[T] = {
    ...
    this.flattened_bitmask.foreach(
      _ :=
        VecInit
          .tabulate(way * dataWidth)(
            n => waymask(n / dataWidth) && bitmask(n % dataWidth)
          )
          .asUInt
    )
...
class SRAMTemplate[T <: Data](
  ...
  val bypass_wdata =
    if (bypassWrite) VecInit(RegEnable(io.w.req.bits.data, io.w.req.valid && io.r.req.valid).map(_.asTypeOf(wordType)))
    else
      VecInit(
        (0 until way).map(
          _ => LFSR64().asTypeOf(wordType)
        )
      )
...
class FoldedSRAMTemplate[T <: Data](
  ...
  val wmask = (width, way) match {
    ...
    case _ =>
      VecInit(
        Seq.tabulate(width * way)(
          n => (n / way).U === widthIdx && io.w.req.bits.waymask.get(n % way)
        )
      ).asUInt
  }
