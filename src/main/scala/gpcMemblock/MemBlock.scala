...
class MemBlockResp extends Bundle {
  val source  = UInt(MasterSource.width.W)
...
class MemBlockIO extends Bundle {
...
abstract class MemBlock(tileId: Int)(implicit p: Parameters) extends LazyModule
...
class MemBlockBundle(val outer: MemBlock)(implicit p: Parameters) extends CoreBundle()(p) {
