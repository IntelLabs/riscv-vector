...
class Vsplit(implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val in = new Bundle {
      ...
      val rfData = if (hasMatrix) Some(Input(Vec(NVPhyRegs, UInt(VLEN.W)))) else None
    }
    val out = new Bundle {
      val mUop = ValidIO(new Muop)
      val toRegFileRead = Output(new regReadIn)
      val mUopMergeAttr = ValidIO(new MuopMergeAttr)
      val mma_in = if (hasMatrix) Some(Output(Valid(new mma_in))) else None
    }
  ...
  if (hasMatrix) {
    ...
    when(cnt_en && (cnt === 1.U)) {
      cnt_en := false.B
    }.elsewhen(io.in.decodeIn.valid && ctrl.matrix) {
      cnt_en := true.B
    }
    ...
    io.out.mma_in.get.bits.srcType := 1.U
    io.out.mma_in.get.bits.dstType := 2.U
   // io.out.mma_in.get.bits.srcType := 6.U
   // io.out.mma_in.get.bits.dstType := 6.U
    ...
    when(cnt === 0.U) {
      io.out.mma_in.get.bits.srcA := Cat(io.in.rfData.get(11)(3 * VLEN / 4 - 1, VLEN / 2),
        ...
        io.in.rfData.get(10)(3 * VLEN / 4 - 1, VLEN / 2),
        io.in.rfData.get(10)(VLEN / 4 - 1, 0),
        io.in.rfData.get(9)(3 * VLEN / 4 - 1, VLEN / 2),
        io.in.rfData.get(9)(VLEN / 4 - 1, 0),
        io.in.rfData.get(8)(3 * VLEN / 4 - 1, VLEN / 2),
        io.in.rfData.get(8)(VLEN / 4 - 1, 0),
      )
      io.out.mma_in.get.bits.srcB := Cat(io.in.rfData.get(15)(3 * VLEN / 4 - 1, VLEN / 2),
        io.in.rfData.get(15)(VLEN / 4 - 1, 0),
        io.in.rfData.get(14)(3 * VLEN / 4 - 1, VLEN / 2),
        io.in.rfData.get(14)(VLEN / 4 - 1, 0),
        io.in.rfData.get(13)(3 * VLEN / 4 - 1, VLEN / 2),
        io.in.rfData.get(13)(VLEN / 4 - 1, 0),
