...
class Vsplit(
  ...
  val log2SrcElen = MuxLookup(
    eewEmulInfo1.veewVs2,
    4.U,
    Seq(
      "b000".U -> 4.U, // VLEN has 16 elements
      "b001".U -> 3.U, // 8
  ...
  val lsrc0_inc = // vcompress
    Mux(
      ctrl.redu || floatRed || (ctrl.funct6 === "b010111".U && ctrl.funct3 === 2.U) || vmaskExcp,
      0.U,
  ...
  io.scoreBoardSetIO.setEn := RegNext(
    fire2PipeReg && ctrl.ldestVal && (~ctrl.perm || ~(ldstCtrl.segment && segmentRegNotFirstElem))
  )
  ...
  expdLenLdSt := Mux(
    ldst && ldstCtrl.segment,
    expdLenSeg,
    Mux(ldstCtrl.wholeReg, eewEmulInfo1.emulVd, Mux(ldst && ldstCtrl.indexed, expdLenIdx, ldStEmulVd)),
  )
  ...
  expdLenIn := Mux(ldst, expdLenLdSt, Mux(ctrl.perm || vmv_vfmv, 1.U, Mux(vcpop || viota || vid, lmul, maxOfVs12Vd)))
