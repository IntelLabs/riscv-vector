...
//   val pmp     = Input(Vec(nPMPs, new GPCPMP))
//   // val customCSRs = Flipped(customCSRs)
// }
...
// class TLBEntry(val nSectors: Int, val superpage: Boolean, val superpageOnly: Boolean)(
//     implicit p: Parameters
// ) extends Bundle {
...
//    * does the refill
//    *
//    * find the target entry with vpn tag and replace the target entry with the
//    * input entry data
//    */
...
//  *
//  * @param nSets
//  *   the number of sets of PTE, follow [[ICacheParams.nSets]]
//  * @param nWays
//  *   the total number of wayss of PTE, follow [[ICacheParams.nWays]]
//  * @param nSectors
//  *   the number of ways in a single PTE TLBEntry
//  * @param nSuperpageEntries
//  *   the number of SuperpageEntries
//  */
// case class GPCTLBConfig(
//     nSets:             Int,
//     nWays:             Int,
//     nSectors:          Int = 4,
...
//  *
//  * TLB caches PTE and accelerates the address translation process. When tlb miss
//  * happens, ask PTW(L2TLB) for Page Table Walk. Perform PMP and PMA check during
//  * the translation and throw exception if there were any.
//  *
//  * ==Cache Structure==
//  *   - Sectored Entry (PTE)
...
//  *
//  * ==Note==
//  * PMA consume diplomacy parameter generate physical memory address checking
//  * logic
//  *
...
//  */
// class GPCTLB(instruction: Boolean, lgMaxSize: Int, cfg: GPCTLBConfig)(
...
//    * Special Entry
//    *
//    * If PMP granularity is less than page size, thus need additional "special"
//    * entry manage PMP.
//    */
...
//    *
//    * @see
//    *   RV-priv spec 4.1.11 Supervisor Address Translation and Protection (satp)
//    *   Register
//    * @see
//    *   RV-priv spec 8.2.18 Virtual Supervisor Address Translation and Protection
//    *   Register (vsatp)
//    */
...
//       special_entry.map(e => e.ppn(vpn, e.getData(vpn))).getOrElse(0.U),
...
//   pmp.io.pmp  := (io.ptw.pmp: Seq[GPCPMP])
//   pmp.io.prv  := mpu_priv
//   // PMA
...
//   val homogeneous =
//     TLBPageLookup(edge.manager.managers, xLen, blockBytes, BigInt(1) << pgIdxBits)(mpu_physaddr).homogeneous
//   // In M mode, if access DM address(debug module program buffer)
//   val deny_access_to_debug =
//     mpu_priv <= PRV.M.U && p(DebugModuleKey).map(dmp => dmp.address.contains(mpu_physaddr)).getOrElse(false.B)
...
//   val ppn = Mux1H(
//     hitsVec :+ !vm_enabled,
//     (all_entries zip entries).map { case (entry, data) => entry.ppn(vpn, data) } :+ vpn(ppnBits - 1, 0),
//   )
...
//   val stage1_bypass =
//     Fill(entries.size, usingHypervisor.B) & (Fill(entries.size, !stage1_en) | entries.map(_.ae_stage2).asUInt)
...
//   val r_array =
//     Cat(true.B, (priv_rw_ok & (entries.map(_.sr).asUInt | Mux(mxr, entries.map(_.sx).asUInt, 0.U))) | stage1_bypass)
...
//   val hr_array =
//     Cat(true.B, entries.map(_.hr).asUInt | Mux(io.ptw.status.mxr, entries.map(_.hx).asUInt, 0.U) | stage2_bypass)
...
//   io.resp.prefetchable := (prefetchable_array & hits).orR && edge.manager.managers.forall(m =>
//     !m.supportsAcquireB || m.supportsHint
//   ).B
...
//    *
//    * If there is a invalid entry, replace it with priorityencoder; if not,
//    * replace the alt entry
//    *
//    * @return
