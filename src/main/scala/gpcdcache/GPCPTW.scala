...
// /**
//  * PTW contains L2TLB, and performs page table walk for high level TLB, and
//  * cache queries from L1 TLBs(I$, D$, RoCC)
//  *
//  * It performs hierarchy page table query to mem for the desired leaf PTE and
//  * cache them in l2tlb. Besides leaf PTEs, it also caches non-leaf PTEs in
//  * pte_cache to accerlerate the process.
//  *
//  * ==Structure==
//  *   - l2tlb : for leaf PTEs
//  *     - set-associative (configurable with [[nL2TLBEntries]]and
//  *       [[nL2TLBWays]]))
//  *     - PLRU
...
//  * }}}
//  * ==State Machine==
//  * s_ready: ready to reveive request from TLB s_req: request mem; pte_cache hit
//  * judge s_wait1: deal with l2tlb error s_wait2: final hit judge s_wait3:
//  * receive mem response s_fragment_superpage: for superpage PTE
//  *
//  * @note
...
//     val aux_ppn_hi = (pgLevels > 4 && r_req.addr.getWidth > aux_pte.ppn.getWidth).option(Reg(
//       UInt((r_req.addr.getWidth - aux_pte.ppn.getWidth).W)
//     ))
...
//     val pte_cache_addr = if (!usingHypervisor) pte_addr
//     else {
...
//     val stage2_pte_cache_addr = if (!usingHypervisor) 0.U
//     else {
//       val vpn_idxs = (0 until pgLevels - 1).map { i =>
...
//         Cat(ppn >> (pgLevelBits * i), r_req.addr(((pgLevelBits * i) min vpnBits) - 1, 0).padTo(pgLevelBits * i))
//       )
...
//     // l2tlb
//     val (l2_hit, l2_error, l2_pte, l2_tlb_ram) = if (nL2TLBEntries == 0)
...
//         val wmask = if (nL2TLBWays > 1)
//           Mux(r_valid_vec_q.andR, UIntToOH(r_l2_plru_way, nL2TLBWays), PriorityEncoderOH(~r_valid_vec_q))
//         else 1.U(1.W)
...
//       val s2_hit_vec =
//         (0 until nL2TLBWays).map(way => s2_valid_vec(way) && (r_tag === s2_entry_vec(way).tag))
...
//         true.B
//       } else {
//         TLBPageLookup(edge.manager.managers, xLen, blockBytes, pgSize)(r_pte.ppn << pgIdxBits).homogeneous
//       }
//     }
...
//             aux_pte.ppn,
//             makeFragmentedSuperpagePPN(aux_pte.ppn)(aux_count),
//           ),
//           gpa_pgoff,
//         )
...
//         Mux(
//           state === s_req && !stage2_pte_cache_hit && pte_cache_hit,
...
//                 makePTE(makeFragmentedSuperpagePPN(r_pte.ppn)(count), r_pte),
...
//         traverse,
//         pte, {
//           val s1_ppns = (0 until pgLevels - 1).map(i =>
//             Cat(
//               pte.ppn(pte.ppn.getWidth - 1, (pgLevels - i - 1) * pgLevelBits),
//               r_req.addr(((pgLevels - i - 1) * pgLevelBits min vpnBits) - 1, 0).padTo((pgLevels - i - 1) * pgLevelBits),
//             )
//           ) :+ pte.ppn
...
//       ccover(
//         leaf && pte.v && !invalid_paddr && pte.reserved_for_future === 0.U,
//         s"L$i",
//         s"successful page-table access, level $i",
//       )
...
//   private def ccover(cond: Bool, label: String, desc: String)(
