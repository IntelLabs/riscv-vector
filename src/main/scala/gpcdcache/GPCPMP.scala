...
//   def apply(reg: PMPReg): GPCPMP = {
//     val pmp = Wire(new GPCPMP()(Parameters.empty))
//     pmp.cfg  := reg.cfg
//     pmp.addr := reg.addr
...
//   val addr = UInt((paddrBits - GPCPMP.lgAlign).W)
...
//   def readAddr = if (pmpGranularity.log2 == GPCPMP.lgAlign) addr
//   else {
//     val mask = ((BigInt(1) << (pmpGranularity.log2 - GPCPMP.lgAlign)) - 1).U
...
//   private def rangeMatch(x: UInt, lgSize: UInt, lgMaxSize: Int, prev: GPCPMP) =
...
//   private def rangeHomogeneous(x: UInt, pgLevel: UInt, prev: GPCPMP) = {
...
//   def homogeneous(x: UInt, pgLevel: UInt, prev: GPCPMP): Bool =
...
//   def aligned(x: UInt, lgSize: UInt, lgMaxSize: Int, prev: GPCPMP): Bool = if (lgMaxSize <= pmpGranularity.log2) true.B
//   else {
//     val lsbMask = UIntToOH1(lgSize, lgMaxSize)
//     val straddlesLowerBound = ((x >> lgMaxSize) ^ (prev.comparand >> lgMaxSize)) === 0.U && (prev.comparand(
//       lgMaxSize - 1,
//       0,
//     ) & ~x(lgMaxSize - 1, 0)) =/= 0.U
//     val straddlesUpperBound = ((x >> lgMaxSize) ^ (comparand >> lgMaxSize)) === 0.U && (comparand(
//       lgMaxSize - 1,
//       0,
//     ) & (x(lgMaxSize - 1, 0) | lsbMask)) =/= 0.U
...
//   def hit(x: UInt, lgSize: UInt, lgMaxSize: Int, prev: GPCPMP): Bool =
//     Mux(napot, pow2Match(x, lgSize, lgMaxSize), torNotNAPOT && rangeMatch(x, lgSize, lgMaxSize, prev))
// }

// class PMPHomogeneityChecker(pmps: Seq[GPCPMP])(
//     implicit p: Parameters
// ) {
//   def apply(addr: UInt, pgLevel: UInt): Bool =
//     pmps.foldLeft((true.B, 0.U.asTypeOf(new GPCPMP))) { case ((h, prev), pmp) =>
//       (h && pmp.homogeneous(addr, pgLevel, prev), pmp)
//     }._1
// }

// class PMPChecker(lgMaxSize: Int)(
...
//   val pmp0    = WireInit(0.U.asTypeOf(new GPCPMP))
//   pmp0.cfg.r := default
//   pmp0.cfg.w := default
//   pmp0.cfg.x := default
