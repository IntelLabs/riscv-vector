...
class GPCDCacheWrapperModule(outer: GPCDCacheWrapper) extends HellaCacheModule(outer) {
  ...
  cacheReq.source := s1_req.tag(0) // only fpu
  cacheReq.dest := s1_req.tag >> 1.U
  ...
  io.cpu.s2_nack := (s2_valid && s2_tlbResp.miss) || 
    ...
    RegNext(isRead(s1_req.cmd)))
  io.cpu.s2_nack_cause_raw := false.B
  io.cpu.s2_uncached := false.B
  io.cpu.ordered := true.B // FIXME

  // cache resp 
  io.cpu.resp.bits := DontCare
  io.cpu.resp.valid := s2_cacheRespValid && s2_cacheResp.status =/= CacheRespStatus.miss
  ...
  io.cpu.resp.bits.tag := Cat(s2_cacheResp.dest, s2_cacheResp.source(0))
