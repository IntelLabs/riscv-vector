...
class Gpc(tile: GpcTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  class GpcImpl { // entering gated-clock domain
    ...
    val m1_reg_uops = RegInit(VecInit.fill(decodeWidthGpc)(0.U.asTypeOf(new SUOp)))
    val m1_reg_valids = RegInit(VecInit(Seq.fill(decodeWidthGpc)(false.B)))
    ...
    val m2_reg_uops = RegInit(VecInit.fill(decodeWidthGpc)(0.U.asTypeOf(new SUOp)))
    val m2_reg_valids = RegInit(VecInit(Seq.fill(decodeWidthGpc)(false.B)))
    ...
    val hazard_targets = id_uops map {
    ...
    when(!id_swap && !id_uops(1).ctrl.pipe1) {
      readys_swapped(1) := false.B
    }
    when(id_swap && !id_uops(0).ctrl.pipe0) {
      readys_swapped(0) := false.B
    }
    ...
    val id_check_xcpt = (0 until 2) map { i =>
      checkExceptions(
        {
          if (i == 0) List((csr.io.interrupt, csr.io.interrupt_cause)) else Nil
        } ++: List(
    ...
    when(ll_wen_mem_p0) {
      io.vwb_ready.wb_int_ready := false.B
    }
    ...
    when(state_vsb === s_vsb_idle) {
      when(vcomplete_xcpt) {
        state_vsb := s_vsb_xcpt
      }
    }.otherwise {
      when(vxcpt_flush) {
        state_vsb := s_vsb_idle
      }
    }
    ...
    for (i <- 0 until 2) {
      ...
      when(rf_wen(i)) {
        rf.write(wb_reg_waddr(i), wb_reg_wdata(i))
      }
    }
  ...
  val gpcImpl = withClock(gated_clock) {
    new GpcImpl
  }
