  ...
  this: AnyFlatSpec with ChiselScalatestTester with BundleGenHelper =>
    ...
    def vLsuTest2(): Unit = {
        ...
        test(new SmartVectorLsuTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
            ...
            dut.clock.step(1)
            val ldReqs = Seq(
                (vluxei16.copy(vl=5, uopIdx=0, uopEnd=false, vsew=3), SrcBundleLd(vs2="h0000ffe800000000fff0ffff"), "h5678901234503489eeeeeeeeeeeeeeee".U),
                (vluxei16.copy(vl=5, uopIdx=1, uopEnd=false, vsew=3), SrcBundleLd(vs2="h0000ffe800000000fff0ffff"), "h0123456789abcdef0123456789abcdef".U),
                (vluxei16.copy(vl=5, uopIdx=2, uopEnd=true, vsew=3),  SrcBundleLd(vs2="h0000ffe800000000fff0ffff"), "h201f1e1d1c1b1a191010101010101010".U),
            )
    ...
    def vLsuTest3(): Unit = {
        ...
        test(new SmartVectorLsuTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
            ...
            dut.clock.step(1)
            val ldReqs = Seq(
                (vluxei32.copy(vl=5, uopIdx=0, uopEnd=false, vsew=1), SrcBundleLd(vs2="h0000001000000000fffffff8"), "h0".U),
                (vluxei32.copy(vl=5, uopIdx=1, uopEnd=true, vsew=1),  SrcBundleLd(vs2="h08"), "h201f1e1d1c1bffffcdef0f0fcdefeeee".U),
            )
    ...
    def vLsuTest4(): Unit = {
        it should "pass: indexed load (uops=2, eew=64, sew=16, vl=16, vstart=0)" in {
        test(new SmartVectorLsuTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
            test_init(dut)
            dut.clock.step(1)
            val ldReqs = Seq(
    ...
    def vLsuTest5(): Unit = {
        ...
        test(new SmartVectorLsuTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
            ...
            val ldReqs = Seq(
                (vluxei32.copy(vl=5, uopIdx=0, uopEnd=false, vsew=1), SrcBundleLd(vs2="h0000001000000000fffffff8"), "h0".U),
                (vluxei32.copy(vl=5, uopIdx=1, uopEnd=true, vsew=1),  SrcBundleLd(vs2="h60"), "h201f1e1d1c1b1a19cdef0f0fcdefeeee".U),
            )
