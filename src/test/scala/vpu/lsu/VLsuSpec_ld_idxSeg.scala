...
trait VLsuBehavior_ld_idxSeg {
  ...
  def vLsuTest1(): Unit = {
    it should "pass: indexed load sew=16" in {
      test(new VLsuTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        test_init(dut)
        dut.clock.step(1)
        val vSew = 1
  ...
  def vLsuTest2(): Unit = {
    it should "pass: indexed load with vstart/vmask" in {
      test(new VLsuTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        test_init(dut)
        dut.clock.step(1)
        val vSew = 1
  ...
  def vLsuTest3(): Unit = {
    it should "pass: indexed load sew=64 with vstart/vmask" in {
      test(new VLsuTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        test_init(dut)
        dut.clock.step(1)
        val vSew = 3
  ...
  def vLsuTest4(): Unit = {
    it should "pass: segment load vlseg2e8" in {
      test(new VLsuTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        ...
        val vSew = 1
        val ldReqs = Seq(
          (vlseg2e8.copy(vl=2, uopIdx=0), ldReqSrc_default),
          (vlseg2e8.copy(vl=2, uopIdx=1, uopEnd=true), ldReqSrc_default),
        )
        val ldResps = Seq(
          ("h0004000300020001_0008000700060005_000c000b000a0009_0001000f000e000d" + 
            "0123456701234567_89abcdef89abcdef_0123456701234567_89abcdef89abcdef",
        ...
        fork {
          ...
          for (i <- 0 until 10) {
            if (dut.io.wb.ld.valid.peekBoolean()) {
              if (wb_cnt == 0) {
                dut.io.wb.ld.bits.vd.expect("hffffffffffff_ffffffffffffffff_ffffffffffffffff_ffffffffffffffff_efef".U)
                dut.io.wb.ld.bits.uop.expdIdx.expect(0.U)
              } else {
                dut.io.wb.ld.bits.vd.expect("hffffffffffff_ffffffffffffffff_ffffffffffffffff_ffffffffffffffff_cdcd".U)
                dut.io.wb.ld.bits.uop.expdIdx.expect(1.U)
              }
              wb_cnt += 1
            }
